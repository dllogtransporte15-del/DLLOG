...
const AppContent: React.FC = () => {
  ...
  const [currentUser, setCurrentUser] = useState<User | null>(() => {
    ...
    const saved = localStorage.getItem('dllog_currentUser');
    return saved ? JSON.parse(saved) : null;
  });
  const [currentPage, setCurrentPage] = useState<Page>(() => {
    return (localStorage.getItem('dllog_currentPage') as Page) || 'dashboard';
  });
  ...
  const [companyLogo, setCompanyLogo] = useState<string | null>(() => {
    return localStorage.getItem('dllog_companyLogo') || DLLOG_LOGO_BASE64;
  });

  const [themeImage, setThemeImage] = useState<string | null>(() => {
    return localStorage.getItem('dllog_themeImage') || null;
  });

  const [nextIds, setNextIds] = useState(() => {
    const saved = localStorage.getItem('dllog_nextIds');
  ...
  useEffect(() => {
    if (currentUser) {
      localStorage.setItem('dllog_currentUser', JSON.stringify(currentUser));
    } else {
      localStorage.removeItem('dllog_currentUser');
    }
  }, [currentUser]);

  useEffect(() => {
    localStorage.setItem('dllog_currentPage', currentPage);
  }, [currentPage]);
  ...
  useEffect(() => { localStorage.setItem('dllog_nextIds', JSON.stringify(nextIds)); }, [nextIds]);
  useEffect(() => {
    if (companyLogo) {
      localStorage.setItem('dllog_companyLogo', companyLogo);
      ...
      localStorage.removeItem('dllog_companyLogo');
    }
  }, [companyLogo]);
  useEffect(() => {
    if (themeImage) {
      localStorage.setItem('dllog_themeImage', themeImage);
    } else {
      localStorage.removeItem('dllog_themeImage');
    }
  }, [themeImage]);
  ...
  const handleSaveThemeImage = async (image: string) => {
    ...
    toast.success("Tema de fundo atualizado com sucesso!");
  };
  ...
  const handleCreateShipment = async (data: NewShipmentRequestData): Promise<boolean> => {
    ...
    if (!defaultOwner) {
        toast.error("Erro crítico: Proprietário padrão para veículos de terceiros não encontrado. Contate o suporte.");
        return false;
    }
    ...
    if (data.filesToAttach && data.filesToAttach.length > 0) {
      try {
        ...
        toast.error('Ocorreu um erro ao enviar os arquivos. O embarque foi criado, mas os arquivos não puderam ser salvos.');
      }
    }
    ...
    try {
      ...
      toast.error(`[ERRO CRÍTICO] O embarque não pôde ser salvo no banco de dados: ${errorMessage}. Verifique sua conexão ou contate o suporte.`);
      return false;
    }

    setCurrentPage('shipments');
    toast.success(`Novo embarque ${newShipmentId} criado com sucesso! Motoristas/Veículos não cadastrados foram adicionados automaticamente.`);
    return true;
  };
  ...
  const handleUpdateShipmentAttachment = async (shipmentId: string, data: { 
    ...
    if (originalShipment.status === ShipmentStatus.AguardandoNota && !originalShipment.bankDetails && !bankDetails) {
        toast.error('Dados bancários são obrigatórios para avançar para a etapa de adiantamento.');
        return;
    }

    if (originalShipment.status === ShipmentStatus.AguardandoCarregamento && !route?.trim()) {
        toast.error('A rota do motorista é obrigatória para avançar para a próxima etapa.');
        return;
    }

    if (originalShipment.status === ShipmentStatus.AguardandoCarregamento && (!loadedTonnage || loadedTonnage <= 0)) {
        toast.error('O peso carregado é obrigatório para avançar para a próxima etapa.');
        return;
    }
    ...
    if (!isUserAllowed) {
        toast.error(`Você não tem permissão para alterar o status deste embarque. ${alertMessage}`);
        return;
    }
    ...
    try {
      ...
      toast.error('Ocorreu um erro ao enviar os arquivos. Tente novamente.');
      return;
    }
  ...
  if (isLoading && shipments.length === 0 && cargos.length === 0) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', gap: '16px', background: '#f9fafb' }}>
        ...
        <p style={{ color: '#6b7280', fontSize: '18px', fontWeight: 500 }}>Carregando DLLOG...</p>
